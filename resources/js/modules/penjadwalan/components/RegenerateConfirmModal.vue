  ...
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-modal-in">
      ...
      <p class="text-sm text-gray-500 text-center mb-4">
        Ditemukan <strong class="text-orange-600">draft jadwal tersimpan</strong> untuk periode dan tipe ini.
        Jika melanjutkan, draft lama akan dihapus dan digantikan dengan jadwal baru.
      </p>

      <!-- Info draft -->
      <div class="bg-orange-50 rounded-xl px-4 py-3 mb-5 flex items-start gap-3">
        <svg class="w-4 h-4 mt-0.5 text-orange-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/>
        </svg>
        <div>
          <div class="text-sm font-bold text-orange-700">Draft Ditemukan</div>
          <div class="text-xs text-orange-600">{{ draftCount }} jadwal — Disimpan {{ formatTime(draftSavedAt) }}</div>
        </div>
      </div>
      ...
      <div class="flex gap-3">
        <button
          ...
          class="flex-1 py-2.5 text-sm font-semibold text-blue-600 border border-blue-200 bg-blue-50 rounded-xl hover:bg-blue-100 transition"
        >
          📂 Lanjutkan Draft
        </button>
        <button
          @click="handleGenerateUlang"
          class="flex-1 py-2.5 text-sm font-bold text-white bg-orange-500 hover:bg-orange-600 rounded-xl transition shadow-sm shadow-orange-200"
        >
          🔄 Generate Ulang
        </button>
      </div>
