...
export default {
  ...
  methods: {
    ...
    async handleExport() {
      this.$store.commit("SET_LOADING", true);
      try {
        const response = await this.$store.dispatch(DISPATCH.EXPORT_MATA_KULIAH, {
          search: this.search || undefined,
        });
        ...
        link.href = url;
        link.setAttribute('download', 'daftar_mata_kuliah_per_prodi.xlsx');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
      } catch (e) {
        console.error("Gagal mengekspor data mata kuliah:", e);
      } finally {
        this.$store.commit("SET_LOADING", false);
      }
    },
